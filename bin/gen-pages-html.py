...
def fetch_builds() -> list[dict[str, Any]]:
    """Fetch ALL builds with pagination via COPR API.

    Returns an empty list on network/parse failure (never raises), which
    matches the old dashboard contract.
    """
    result: list[dict[str, Any]] = []
    offset = 0
    while True:
        url = (f"{API_BASE}?ownername={OWNER}&projectname={PROJECT}"
               f"&limit={PAGE_LIMIT}&offset={offset}")
        try:
            resp = urllib.request.urlopen(url, timeout=40)
            data: dict[str, Any] = json.loads(resp.read())
        except (urllib.error.URLError, json.JSONDecodeError, OSError) as e:
            print(f"api error @offset={offset}: {e}", file=sys.stderr)
            return []
...
<p class="dim"><code>sudo zypper addrepo</code> (см. README beta)</p>
</div>
<script>
